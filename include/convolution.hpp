...
namespace fft {
    ...
    /// @param inverse If true apply the inverse Fourier transform instead.
    /// @pre size(a) is a power of 2
    template<typename T>
    ...
    /// @brief Perfrom a convolution with fft by cutting each element to real and imaginary parts.
    ///        Requires one additional fourier transform and more ring multiplications but 
    ...
    std::vector<T> convolution_wcut(std::vector<T> const &a, std::vector<T> const &b) {
        ...
        for(int i = 0; i < na; i++) {
            T x0 = T(a[i] / cut), x1 = a[i] - x0;
            Az[i] = complex<ftype>(x0, x1);

        }

        for(int i = 0; i < nb; i++) {
            T x0 = T(b[i] / cut), x1 = b[i] - x0;
    ...
    /// @return The convolution of a and b using FFT.
    ...
    /// @brief Apply the Fourier transform to a.
    /// @tparam F A field.
    /// @param a The sequence to be transformed.
    /// @pre size(a) is a power of 2
    /// @param root The roots of unity of F.
    /// @param iroot iroot are the inverses of the roots of unity, i.e. iroot[k] = 1 / root[k].
    /// @param inverse If true apply the inverse Fourier transform instead.
    template<typename F>
    ...
    /// @return The convolution of a and be using FFT.
    template<typename T>
...
/// @brief Perform the convolution of a and b naively, i.e. in O(size(a) * size(b)) operations.
/// @return The convolution of a and b.
template <typename T>
